<template>
  <div>
    ...
    <div @dragover.prevent>
      <div class="flex items-center justify-between" v-for="item in city" :key="city">
        <!-- Drop icon -->
        <div class="bg-red-500 inline-block rounded-lg p-1 mr-1 pb-4"></div>

        <!-- item content -->
        <div class="pr-28 pl-4">
          <p>{{ item.name }}</p>
        </div>

        <!-- Remove item icon -->
        <div class="h-4 w-4" @click="removeLocation(city)">
...
function removeLocation(item: number) {
  ...
  // // Remove location
  // let locations = city.filter((e) => e.id !== item.id);
